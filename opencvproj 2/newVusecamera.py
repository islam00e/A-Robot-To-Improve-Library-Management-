...
num_frames_to_capture = 1

# Initialize list to store captured frames
captured_frames = []
...
gray = cv2.cvtColor(captured_frames[0], cv2.COLOR_BGR2GRAY)

# Apply Gaussian blur to reduce noise
blur = cv2.GaussianBlur(gray, (5, 5), 0)

# Apply adaptive thresholding with a larger block size and constant
...
for contour in contours:
    ...
    if h > 50 and w > 50:  # Filter out small contours
        ...
        text = pytesseract.image_to_string(roi_resized, config='--psm 6')  # Use single block mode for better accuracy
